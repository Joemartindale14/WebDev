...
export const ClassBookingProvider = ({ children }) => {
    ...
    useEffect(() => {
        const fetchClasses = async () => {
            const response = await axios.get(process.env.HOST_URI+'/api/classes');
    ...
    const bookClass = async (classId) => {
        await axios.post(process.env.HOST_URI+'/api/book', { classId });
        setBookings((prevBookings) => [...prevBookings, classId]);
    };

    const cancelBooking = async (classId) => {
        await axios.post(process.env.HOST_URI+'/api/cancel', { classId });
